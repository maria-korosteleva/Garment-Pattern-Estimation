    ...
    def standardize(self, training=None):
        ...
        if 'standardize' in self.config:
            ...
            for batch in loader:
                ...
                edge_shift, edge_scale = self._get_distribution_stats(gt['outlines'][:, :, :, :2], padded=True)
                # NOTE mean values for panel edges are zero due to loop property 
                # panel components SHOULD NOT be shifted to keep the loop property intact 
                edge_shift[0] = edge_shift[1] = 0
                curv_shift, curv_scale = self._get_norm_stats(gt['outlines'][:, :, :, 2:], padded=True)
                
                panel_shift, panel_scale = torch.cat([edge_shift, curv_shift]), torch.cat([edge_scale, curv_scale])
